...
$(".deleteButton").click(function(){
	if(confirm("seguro que deseas eliminar la sucursal?")){
		$.ajax({
			data:"SUCU_ID="+$("input[name='sucu_id']").val(),
			url:SERVER_URL_BASE+"sucursales/csucursales/deleteSucursales",
			method:"POST",
			success: function(msg){
				var resp=msg.split(";");
				if(resp[0].trim()=="SUCCESS"){
				alert("Sucursal eliminada correctamente");
				window.location=SERVER_URL_BASE+"sucursales/csucursales/formSelectSucursales";
				}else{
					alert("Ocurrio un error: "+msg);
				}
			}
