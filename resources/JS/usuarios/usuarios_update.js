...
$(".deleteButton").click(function(){
	if(confirm("seguro que deseas eliminar el usuario?")){
		$.ajax({
			data:"USR_ID="+$("input[name='usr_id']").val(),
			url:SERVER_URL_BASE+"usuarios/cusuarios/deleteUsuarios",
			method:"POST",
			success: function(msg){
				var resp=msg.split(";");
				if(resp[0].trim()=="SUCCESS"){
				alert("Usuario eliminado correctamente");
				window.location=SERVER_URL_BASE+"usuarios/cusuarios/formSelectUsuarios";
				}else{
					alert("Ocurrio un error: "+msg);
				}
			}
